async function loadData() {
    ...
    if (response.ok) {
        ...
        //load condition list
        loadOption("quality", json.qualityList, "name");
...
async function searchProducts(firstResult) {
    ...
    let quality_name = document.getElementById("quality-options")
            .querySelector(".chosen")
            ?.querySelector("a").innerHTML;
...
function updateProductView(json) {
    ...
    json.productList.forEach(product => {
        ...
        st_product_clone.querySelector("#st-product-a").href = "shop-details.html?id=" + product.id;
        st_product_clone.querySelector("#st-product-img-1").src = "product-images/" + product.id + "/image1.png";
        st_product_clone.querySelector("#st-product-a2").href = "shop-details.html?id=" + product.id;
